...
```
Since there are many vuales of the explanatory variable "age" in the original data, we've decided to mutate the variable into age categories from under 20 to over 70 in incriments of 10 years. 
```{r}
library(survival)
library (survminer)
library(ggplot2)
...
#full model
cph_full<- coxph(Surv(time,censor)~., data = aids)
cph_full$loglik
cph_full

#reduced model 1
cph_r1 <- coxph(Surv(time,censor)~.-priorzdv, data = aids) 
cph_r1$loglik
cph_r1
...
#reduced model 2
cph_r2 <- coxph(Surv(time,censor)~.-priorzdv -id, data = aids)
cph_r2$loglik
cph_r2
...
#reduced model 3
cph_r3 <- coxph(Surv(time,censor)~.-priorzdv -id -hemophil, data = aids)
cph_r3$loglik
cph_r3
...
#reduced model 4
cph_r4 <- coxph(Surv(time,censor)~.-priorzdv -id -hemophil -raceth, data = aids)
cph_r4$loglik
cph_r4
...
#reduced model 5
cph_r5 <- coxph(Surv(time,censor)~.-priorzdv -id -hemophil -raceth -time_d, data = aids)
cph_r5$loglik
cph_r5
...
#reduced model 6
cph_r6 <- coxph(Surv(time,censor)~.-priorzdv -id -hemophil -raceth -time_d -strat2, data = aids)
cph_r6$loglik
cph_r6
...
#reduced model 7
cph_r7 <- coxph(Surv(time,censor)~.-priorzdv -id -hemophil -raceth -time_d -strat2 -sex, 
                data = aids)
cph_r7$loglik
cph_r7
...
#reduced model 8
cph_r8 <- coxph(Surv(time,censor)~.-priorzdv -id -hemophil -raceth -time_d -strat2
                -sex -txgrp -age, data = aids)
cph_r8$loglik
cph_r8
...
#reduced model 9
cph_r9 <- coxph(Surv(time,censor)~.-priorzdv -id -hemophil -raceth -time_d -strat2 -sex 
                -txgrp -age -tx, data = aids)
cph_r9$loglik
cph_r9
...
#reduced model 10
cph_r10 <- coxph(Surv(time,censor)~.-priorzdv -id -hemophil -raceth -time_d -strat2 -sex 
                 -txgrp -age -tx -censor_d, data = aids)
cph_r10$loglik
cph_r10
...
simoutput <- c()
for(i in 1:n.reps){
  simdata <- sim.survdata(N=1000, T=100, num.data.frames=1, xvars=4,beta = c(0,0.01,0.07,0.3))
