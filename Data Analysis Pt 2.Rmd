...
As we know from lecture, the Cox PH model assumes that $h(t)$ of the $i^{th}$ individual satisfies: 

1) $h_i(t)=h_0(t)e^{z_i(t)\beta}$
      where: 
      $h_0$ -> baseline hazard
...
---
Due to the fact that that some observations might be censored and in particular, regarding the Cox PH model, the baseline hazard is not estimated, in oprder to analyse the residuals a particular score process. The risk score for unit $i$ at time $t$ is thought to be $r_i(t)=e^{z_i(t)\beta}$, where $Y_i(t)$ is the indicator function and $Y_i(t)=1$ indicates a poitn in which $i$ is under risk and thus observation and it is equal to 0 in other occasions.

---

The Schoenfeld residuals are given by the equations:
1) $s_k=Z_{(k)}-\sum_iY_i(t_k)r_i(t_k)Z_i(t_k)/\sum_iY_i(t_k)r_i(t_k)$
...
$V(\beta,t_k)=\sum_iY_i(t_k)r_i(t_k)Z_i(t_k)-\bar{z}(\hat{\beta},t_k)'Z_i(t_k)-\bar{z}(\hat{\beta},t_k)/\sum_iY_i(t_k)r_i(t_k)$
