...
The significance of these variables in our Cox PH model allows us to conclude that there is evidence to suggest that the initial state of health of the individual and their baseline cd4 cell count are significant factors in determing the time it will take an individual to get diagnosed with AIDS or die. The treatment indicator variable, *tx*, had a $p$-value that was not significant. Therefore, we can conclude that being in the treatment group versus being in the control group does not have a significant effect on the time until an AIDS diagnosis or death for an individual. 
...
The *cox.zph()* function provides a Goodness-of-Fit (GOF) test, which tests the correlation between Schoenfeld resuduals and survival time. Here, taking with an alpha level of $\alpha=0.05$ the p-value below this would rejuct the null of independence, thus indicating that the residuals are infact dependent on time and thus the PH assumption is not satisfied. If the p-value is greater than $\alpha$, the null of independence is accepted and thus the PH assumption is met. 

A graphical represenation is achieved using either *ggcoxzph()* or *ggcoxdiagnostics()* functions, that overall should show no pattern in the graphs in order to indicate a PH assumption. In the *ggcoxzph()* function, the solid line is a smoothing spline fit to the plot, with the dashed lines representing a +/- 2-standard-error. Whereas, in *ggcoxdiagnostics()* function graph, the dashed blue line represents the fit to the plot, via red dashed line representing the y=0 point of reference and the grey area aroud the blue line representing  thhe +/- 2-standard-error.
```{r}
...
# Conclusion
Overall, the purpose of this paper was to
